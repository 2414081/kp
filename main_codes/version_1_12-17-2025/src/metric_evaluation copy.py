...
async def evaluate_prompts(file_path, excel_output_folder, selected_prompts):
    ...
    output_file = f"{excel_output_folder}/metric_evaluation_results_2_sample.xlsx"
    ...
    for entry_idx, entry in enumerate(dataset, start=1):
        ...
        if "groundtruth_accuracy_prompt" in selected_prompts:
            ...
            tasks["groundtruth_accuracy_prompt"] = get_llm_response(prompt)
        ...
        if "hallucination_cove_prompt" in selected_prompts:
            ...
            if not missing:
                ...
                tasks["hallucination_cove_prompt"] = get_llm_response(prompt)
            else:
        ...
        if "hallucination_consistency_prompt" in selected_prompts:
            ...
            tasks["hallucination_consistency_prompt"] = get_llm_response(prompt)

        # Run all tasks in parallel
        evaluations = await asyncio.gather(*tasks.values(), return_exceptions=True)
        evaluations = dict(zip(tasks.keys(), evaluations))

        # Process & append results
