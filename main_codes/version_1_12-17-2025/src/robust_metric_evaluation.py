...
def add_robustness_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for parent, group in df.groupby(df[PARENT_COL].str.extract(r"(PT_\d+)_")[0]):
        ...
        # Check if base row passed or failed
        base_passed = status_numeric.loc[base_rows.index].iloc[0] == 1 if not base_rows.empty else True

        if not child_rows.empty:
            child_statuses = status_numeric.loc[child_rows.index]
            
            if base_passed:
                # Base passed: calculate pass rate (pass count / total children)
                score = child_statuses.mean()
            else:
                # Base failed: calculate fail rate (fail count / total children)
                score = (1 - child_statuses).mean()
        else:
            score = 0

        group_scores[parent] = round(score, 2)

        # Debugging scores
        print(f"Parent: {parent}")
        print(f"Base passed: {base_passed}")
